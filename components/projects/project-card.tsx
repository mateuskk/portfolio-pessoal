  ...
  return (
    ...
    >
      ...
      <div className="grid content-between gap-12 p-6 sm:p-8">
        <div className="flex items-start justify-between gap-5 text-label uppercase text-muted">
          <span>{project.index}</span>
          <span>{project.year}</span>
        </div>
        <div>
          <h3 id={headingId} className="font-serif text-5xl leading-none tracking-[-0.055em] sm:text-6xl">{project.title}</h3>
          <p className="mt-4 text-sm uppercase tracking-[0.14em] text-muted">{project.role}</p>
          <p className="mt-7 max-w-lg leading-relaxed text-muted">{project.summary}</p>
          ...
          {project.href ? (
            ...
            <span className="mt-9 inline-block text-label uppercase tracking-[0.16em] text-muted">Case study / Soon</span>
          )}
        </div>
      </div>
    </article>
